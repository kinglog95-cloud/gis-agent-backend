...
def query_osm_lines(lat, lon, radius_meters, line_category):
    ...
    Returns a list of dicts: {'coords': [(lon,lat), ...], 'name', 'kind'}."""
...
out geom;"""
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Overpass lines HTTP {response.status_code}: {response.text[:200]}")
            return []
        ...
        print("Overpass lines error: request timed out")
        return []
    except Exception as e:
        print(f"Overpass lines error: {e}")
        return []
...
def generate_map(lat, lon, radius_meters, features, location_name, category,
                 user_gdf=None, user_filename=None, exposure=None):
    ...
    <div style='position:fixed;top:15px;right:15px;background:white;
                ...
                min-width:250px;font-family:Arial;
...
def proximity_exposure_analysis(lat, lon, radius_m, point_category,
    ...
    line_feats  = query_osm_lines(lat, lon, radius_m, line_category)
...
def generate_static_map(lat, lon, radius_meters, features, location_name,
    ...
    if is_exposure:
        if exp_buffer_3857 is not None:
            try:
                exp_buffer_3857.plot(ax=ax, facecolor='#1f6feb', alpha=0.12,
                                     edgecolor='#1f6feb', linewidth=1.0,
                                     linestyle='--', zorder=2)
            except Exception as e:
...
def analyze():
    ...
    if is_multipart:
        ...
        try:
            ...
            radius_km = 2.0
        compare_against = (request.form.get('compare_against') or '').strip()
        try:
        ...
        try:
            ...
            radius_km = 2.0
        compare_against = (data.get('compare_against') or '').strip()
        try:
...
def health():
    return jsonify({'status': 'ok', 'message': 'GIS Agent Backend running ✅ (V3.5b — exposure analysis on map + PDF)'})
