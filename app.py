...
def _attach_handle(resp_dict, summary_override=None):
    """Given a tool's response dict that contains a 'geojson' key, store that
    geojson in the handle store and add 'handle' + 'summary' to the response.
    Returns the same dict (mutated). Safe no-op if there's no geojson."""
...
def tool_spatial_join():
    ...
    return jsonify(_attach_handle({
        ...
        'outside':    n_out,
        'pct_inside': pct_in,
        'geojson':    _gdf_to_geojson(pts_gdf),
...
def health():
    return jsonify({'status': 'ok', 'message': 'GIS Agent Backend running ✅ (V3.7f — agent session layer: handles on all tools)'})
