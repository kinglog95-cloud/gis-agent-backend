...
def tool_parse_file():
    ...
    return jsonify({
        'success': True,
        'summary': summarize_gdf(gdf, file_name, original_crs),
        'geojson': _gdf_to_geojson(gdf),
    })
...
def health():
    return jsonify({'status': 'ok', 'message': 'GIS Agent Backend running ✅ (V3.7b — agent tools pass 2: + parse_file, aggregate_by_polygon, compute_metrics)'})
